...
const PatientDetails: React.FC = () => {
  ...
  const fetchAppointments = () => {
    ...
    const apiUrl = `${
      process.env.NEXT_PUBLIC_REPORTS_API_BASE_URL || "https://api.edencdc.org"
    }/reports/patient_history/${patientIdStr}?patient_id=${patientIdStr}&date_from=${businessDateStr}&date_to=${businessDateStr}&organisation_id=${
      user.organisation.id
    }`;
        ...
        rawAppointments.forEach((dateGroup: any) => {
          ...
          logs.forEach((log: any, index: number) => {
            mappedAppointments.push({
              ...
              category_name:
                log.DeptCategory?.name ||
                (typeof log.DeptCategory === "string"
                  ? log.DeptCategory
                  : "") ||
                "",
              status: log.status || "",
                ...
                <div className="glass-card rounded-xl p-6 mb-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-4">
                    {appointments.map((appointment: any) => {
                      const therapistName =
                        appointment.consultant_name || "GROUP SESSION";
                      const department =
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-start justify-between">
                            <div>
                              <h3 className="font-medium text-medical-darkGray">
                                {therapistName}
                              </h3>
                              <p className="text-sm text-medical-text">
                                {therapistName === "GROUP SESSION"
                                  ? appointment.group ||
